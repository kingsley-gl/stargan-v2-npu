...
class Solver(nn.Module):
    ...
    def sample(self, loaders):
        ...
        src = next(InputFetcher(loaders.src, self.device, None, args.latent_dim, 'test'))
        ref = next(InputFetcher(loaders.ref, self.device, None, args.latent_dim, 'test'))

        fname = r'/'.join([args.result_dir, 'reference.jpg'])
        print('Working on {}...'.format(fname))
        utils.translate_using_reference(nets_ema, args, src.x, ref.x, ref.y, fname)

        fname = r'/'.join([args.result_dir, 'video_ref.mp4'])
        print('Working on {}...'.format(fname))
